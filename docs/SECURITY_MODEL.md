...
- Renderer 负责 UI、文字输入、现有本地麦克风与演示播放；没有 Node、文件系统、自由网络
  或完整凭据能力。
- Preload 只公开命名、强类型的 health、Provider 配置、Conversation streaming 和 STT
  配置/转录 API；不公开通用 `invoke`、`send` 或 channel 名称。
...
`provider-config.v1.json` 与 `speech-config.v1.json` 位于 Electron userData，不位于
仓库。配置文件只包含：
...
当前文字回答只保留在 Renderer 内存，刷新后消失。JAR-006B 不写 SQLite、不提取认知，
也不接通真实 TTS。Mock STT 不上传音频；real STT 只把用户主动结束的当前录音发送给其
